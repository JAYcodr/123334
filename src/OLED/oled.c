...
{
	OLED_IIC_Start();
	OLED_IIC_Send_Data(0x78);//地址
	OLED_IIC_Send_Data(0x00);//写指令 第6位控制写数据/命令
	OLED_IIC_Send_Data(com);//指令
	OLED_IIC_Stop();
}
...
void Oled_Write_Data(u8 data)
{
	OLED_IIC_Start();
	OLED_IIC_Send_Data(0x78);//地址
	OLED_IIC_Send_Data(0x40);//写数据
	OLED_IIC_Send_Data(data);//指令
	OLED_IIC_Stop();
}
